...
public class ObjectDetectionController {
	...
	NaverService service; //객체감지 서비스.
	
	@RequestMapping("/objectinput")//파일리스트 이미지만가져옴.
	public ModelAndView objectinput() {
		...
		mv.setViewName("objectinput");//.jsp
		return mv;
	}
	...
	@RequestMapping("/objectresult")//json결과 jsp보내주기위해 모델앤뷰
	public ModelAndView objectresult(String image) {
		String objectresult = service.test(image);//테스트메서드, 감지대상넣으면됨.//리턴결과 String
		ModelAndView mv = new ModelAndView();
