...
public class FaceServiceImpl2 implements NaverService {
	...
	public String test(String image) { 
			StringBuffer response  = null; //응답받아 리스폰스 만들어주는형태 --여기다 선언 추가.  
	        ...
	        try {
	            ...
	            String imgFile = MyNaverInform.path +image;  //여기바꿈, 컨트롤러한테 받을거임.
	            ...
	            String inputLine;
	            if(br != null) {
	                response = new StringBuffer();//이부분선언위로올림.
	                while ((inputLine = br.readLine()) != null) {
	                    response.append(inputLine);
	                }
	                br.close();
	                System.out.println(response.toString());//toString이 결과 jason형태 { {객체}, {배열}}//선언이 전지역에서사용할수있게위에선언
	            } else {
